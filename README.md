...
| 数式 | INDEX / MATCH / XLOOKUP / LOOKUP / INDIRECT なども含め、**Excel が保存した計算結果をそのまま表示**（再計算はしない） |
...
| 印刷設定 | 用紙サイズ・向き・余白を `@page` に反映。手動改ページ、印刷タイトル行（各ページで見出しを繰り返す）、`fitToWidth`／拡大縮小率も反映 |
...
| `--gridlines auto\|on\|off` | `auto` | 目盛線。auto は Excel 側の「目盛線の表示」設定に従う |
