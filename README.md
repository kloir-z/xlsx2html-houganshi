...
python xlsx2html.py 入力.xlsx --formula-balloons    # 数式を吹き出しで表示（数式チェック用）
```
...
| 数式 | INDEX / MATCH / XLOOKUP / LOOKUP / INDIRECT なども含め、**Excel が保存した計算結果をそのまま表示**（再計算はしない）。`CELL("filename")` でシート名を出す式だけは例外的に解決する。`--formula-balloons` を付けると**数式そのものを吹き出しで確認**できる |
...
- 画面下部の**「見切れに印」**をオンにすると、切れているセルの端に細い赤線が付き、どこが切れているか一目で分かります

## 目盛線

Excel の目盛線（薄いグレーの線）は、Excel 側の「目盛線の表示」設定に従って描画します。

- 画面下部の**「目盛線」**チェックボックスで、閲覧時に表示／非表示を切り替えられます（切っても方眼の位置はずれません）
...
| `--formula-balloons` | — | 数式セルの下に数式を吹き出しで常時表示する（数式チェック用。後述） |
| `--formula-tips` | — | 数式セルにマウスを乗せると数式を表示する（ブラウザ標準のツールチップ） |
| `--show-formula` | — | **計算結果が保存されていない**数式セルに、数式そのものを表示する |
...
`--show-formula` が数式を出すのは、この **2 の場合（計算結果が保存されていないセル）だけ**です。
結果が保存されている普通のファイルでは値が出るので、数式そのものを見たいときは次の
`--formula-balloons` を使ってください。

### 数式を吹き出しで確認する（`--formula-balloons`）

```
python xlsx2html.py 入力.xlsx --formula-balloons
```

数式セルの下に、数式を吹き出しで出します。値の表示は変わらないので、**値と式を並べて見比べられます**。

長い数式や改行の入った数式を読めるように、次のようにしてあります。

- **小さめの等幅フォント**（10px）で、セルの書式には影響されません
- **長い数式は折って表示**します。折る位置は引数区切り（`,`）→ 演算子（`&` `+` `*` など）→
  括弧の中身、の順に選び、括弧を開いたときは 1 段字下げします。1 行に収まる式はそのまま 1 行です
- **元から改行が入っている数式**（Excel で Alt+Enter を使って整形したもの）は、その改行を尊重します
- **色分け**します。文字列・数値・関数名・セル参照・シート名・エラー値、そして
  **括弧は深さごとに色を変える**ので、どの `(` とどの `)` が対応しているか追えます
- 6 行を超える式は畳んで出し、**マウスを乗せると全文**が出ます（クリックで固定、Esc で全部閉じる）
- **重なっても完全には隠れません。** 読み込み時に重なりを調べて、後ろに回るもの（下の行・
  右の列）を必要な分だけ下へずらし、少なくとも上端が覗くようにします。ずらした吹き出しは
  元のセルまで細い引き出し線でつながります。マウスを乗せたものが最前面に出ます

**同じ式のコピーはまとめます。** 上または左のセルと同じ式（相対参照のずれを補って比較します）は
繰り返しとみなし、**先頭のセルだけに吹き出しを出して `×12` のようにセル数を添えます**。
方眼紙の表では同じ式が何十行も続くので、これが無いと画面が吹き出しで埋まります。

閲覧時は画面下部（シートタブの並び）のチェックボックスで切り替えられます。

| チェックボックス | 既定 | 効果 |
| --- | --- | --- |
| 数式の吹き出し | オン | 吹き出しをまとめて表示／非表示する |
| 同じ式も出す | オフ | まとめた繰り返しの分も全部出す |
| 数式セルに印 | オフ | 数式が入っているセルの右上に赤い三角を付ける |

吹き出しは印刷（Ctrl+P）にもそのまま出ます。式ではなく値だけを印刷したいときは、
「数式の吹き出し」をオフにしてから印刷してください。

**違う式が縦に密集している**シート（同じ式のまとめが効かない場合）では、下へのずらしが積み上がって
吹き出しが元のセルからかなり離れることがあります。引き出し線でつながってはいますが、
読みにくいときは「数式の吹き出し」をオフにして値を確認してください。
表示される吹き出しが 3000 個を超えるシートでは、ずらしの計算自体を省きます（重なったままになります）。

`--formula-tips` とは併用できません（同じ数式が二重に出るため）。ブラウザ標準のツールチップで
足りる場合は `--formula-tips`、数式そのものを読みたい場合は `--formula-balloons` を使ってください。
...
- `--formula-balloons` で、値と数式を並べて見ながら**数式をレビューする**
